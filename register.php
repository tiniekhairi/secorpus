<?php
  ...
  // Check for submit
  if (filter_has_var(INPUT_POST, 'submit')){        //if submit button is clicked
    // Get form data
    ...
    if (!empty($id) && !empty($username) && !empty($name) && !empty($dep) && !empty($email) && !empty($phone) && !empty($password)){
      ...
      if (filter_var($email, FILTER_VALIDATE_EMAIL) === false){
        ...
        $sql = "INSERT INTO `user` (`userID`, `username`, `fullname`, `status`, `email`, `phoneNumb`, `password`)
        VALUES ('$id', '$username', '$name','$dep', '$email', '$phone','$hashedPwd')";
              ...
              <form class="col s12" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" novalidate>
                ...
                <div class="row">
                  <p class="center-align">
                    Already register? <a href="index.php">Login</a><br><br>
